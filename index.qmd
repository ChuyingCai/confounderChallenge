...
COLOR_DREAMY = "#4E79A7"
COLOR_DUCK = "#E15759"
LABEL_DREAMY = "Doc Dreamy"
LABEL_DUCK = "Doc Duck"

dreamy = patients_df[patients_df["doctor_name"] == LABEL_DREAMY]
duck = patients_df[patients_df["doctor_name"] == LABEL_DUCK]
...
ax.axhline(mean_dreamy, color=COLOR_DREAMY, linestyle="--", linewidth=1.4, alpha=0.85, zorder=2)
ax.axhline(mean_duck, color=COLOR_DUCK, linestyle="--", linewidth=1.4, alpha=0.85, zorder=2)

trans_axes_x = blended_transform_factory(ax.transAxes, ax.transData)
ax.text(
    0.99,
    mean_dreamy,
    f"○ mean = {mean_dreamy:.2f}",
    transform=trans_axes_x,
    ha="right",
    va="center",
    fontsize=11,
    color=COLOR_DREAMY,
...
ax.set_title("Post-Surgical Symptom Score by Patient and Doctor", fontsize=14, fontweight="semibold", pad=10)
ax.set_xlim(0.5, 100.5)
ax.grid(True, linestyle=":", linewidth=0.8, alpha=0.65, zorder=0)
...
dreamy = patients_randomized_df[patients_randomized_df["doctor_name"] == LABEL_DREAMY]
duck = patients_randomized_df[patients_randomized_df["doctor_name"] == LABEL_DUCK]
...
ax.set_xlabel("Patient Number", fontsize=12)
ax.set_ylabel("Post-Surgical Symptom Score (Lower is Better)", fontsize=12)
ax.set_title("Post-Surgical Symptom Score (Randomized Assignment)", fontsize=14, fontweight="semibold", pad=10)
ax.set_xlim(0.5, 100.5)
ax.grid(True, linestyle=":", linewidth=0.8, alpha=0.65, zorder=0)
ax.tick_params(axis="both", labelsize=10)
ax.legend(loc="upper right", framealpha=0.95, fontsize=10)
...
LABEL_DREAMY = "Doc Dreamy"
LABEL_DUCK = "Doc Duck"
OVERLAY_SEVERITY_MIN = -1
OVERLAY_SEVERITY_MAX = 1
OVERLAY_ALPHA = 0.15
OVERLAY_COLOR = "gray"
...
ax.axvspan(
    OVERLAY_SEVERITY_MIN,
    OVERLAY_SEVERITY_MAX,
    alpha=OVERLAY_ALPHA,
    color=OVERLAY_COLOR,
    zorder=0,
)
...
ax.set_ylabel("Post-Surgical Symptom Score (Lower is Better)", fontsize=12)
ax.set_title("Post-Surgical Symptom Score by Patient Severity", fontsize=14, fontweight="semibold", pad=10)
sev = patients_df["severity"]
ax.set_xlim(sev.min() - 0.25, sev.max() + 0.25)
